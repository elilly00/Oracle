...
-- 6. EMPLOYEE테이블에서 이름, 연봉, 총수령액(보너스 포함), 실수령액(총수령액-(연봉*세금 3%)) 조회
...
SELECT *
FROM EMPLOYEE
WHERE HIRE_DATE BETWEEN '90/01/01' AND '01/01/01';  -- BETWEEN AND : 하향값 이상 상한값 이하
...
-- 14. EMPLOYEE테이블에서 메일주소의 '_'의 앞이 4자이면서 DEPT_CODE가 D9 또는 D6이고
--     고용일이 09/01/01~00/12/01이고, 급여가 270만 이상인 사원의 전체를 조회
SELECT *
FROM EMPLOYEE
...
-- 20. EMPLOYEE테이블에서 사원명, 급여 조회(단, 급여는 '\9,000,000' 형식으로 표시)
SELECT EMP_NAME, TO_CHAR(SALARY, 'FML9,999,999') 급여
FROM EMPLOYEE;

-- 21. EMPLOYEE테이블에서 직원명, 부서코드, 생년월일, 나이(만) 조회 (단, 생년월일은 주민번호에서 추출해서 00년 00월 00일로 출력되게, 나이는 주민번호에서 출력해서 날짜데이터로 변환한 다음 계산)

SELECT EMP_NAME 직원명, DEPT_CODE 부서코드, SUBSTR(EMP_NO, 1, 2)||'년 ' || SUBSTR(EMP_NO, 3, 2)||'월 ' || SUBSTR(EMP_NO, 5,2)||'일' 생년월일, EXTRACT(YEAR FROM SYSDATE)-(SUBSTR(EMP_NO, 1, 2)+1900) 나이
FROM EMPLOYEE;
-- 22. EMPLOYEE테이블에서 부서코드가 D5,D6,D9인 사원만 조회하되 
--      D5이면 총무부, D6이면 기획부, D9면 영업부로 처리 (단, 부서코드 오름차순으로 정렬)
SELECT EMP_ID, EMP_NAME, DEPT_CODE, CASE DEPT_CODE WHEN 'D5' THEN '총무부'  WHEN 'D6' THEN '기획부' WHEN 'D9' THEN '영업부' END 부서
FROM EMPLOYEE
WHERE DEPT_CODE IN('D5', 'D6', 'D9');
 
-- 23. EMPLOYEE테이블에서 사번이 201번인 사원명, 주민번호 앞자리, 주민번호 뒷자리, 주민번호 앞자리와 뒷자리의 합 조회
SELECT EMP_ID, SUBSTR(EMP_NO, 1, 6) "주민 앞자리", SUBSTR(EMP_NO, 8, 7) "주민 뒷자리", SUM((SUBSTR(EMP_NO, 1, 6)) + (SUBSTR(EMP_NO, 8, 7))) 총합
FROM EMPLOYEE
...
-- 25. EMPLOYEE테이블에서 직원들의 입사일로부터 년도만 가지고 각 년도별 입사 인원 수 조회
--     전체 직원 수, 2001년, 2002년, 2003년, 2004년
SELECT SUM(COUNT(HIRE_DATE)) "전체 직원 수", COUNT(DECODE(TO_CHAR(EXTRACT(YEAR FROM HIRE_DATE)), '2001', 1)) "2001년",
